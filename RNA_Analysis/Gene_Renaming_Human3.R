...
make_df <- function(dir) {
  ...
  colnames(x1) <- c('gene_id', '07', '08', '09', '10', '11', '12')
  return(x1)
}
...
query_ensembl <- function(df, dir) {
  ...
  df <- select(df, 'gene_name', '07', '08', '09', '10', '11', '12')
...
dir <- "/media/patrick/Elements/Hyperthermia/Human_3/Counts/"
x <- make_df(dir)
...
dir_2 <- dir <- "/media/patrick/Elements/Hyperthermia/Human_3/CountMatrix/"
f_save <- paste(dir_2, 'human_3_counts.csv', sep='')
write.csv(x, f_save)
