...
dds <- DESeq(dds)
res   <- results(dds, alpha=0.1, contrast = c('condition', 'Heat', 'Ctrl')) #, alpha=0.1, contrast=c('condition','Heat','Ctrl'))
#rlog <- rlog(dds)
vst <- vst(dds)
...
save(res, file=paste('/media/patrick/Elements/Hyperthermia/Figures_Human_GBM/','RES',   sep=''))
