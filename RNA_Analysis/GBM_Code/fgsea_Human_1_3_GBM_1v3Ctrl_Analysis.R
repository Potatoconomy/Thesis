...
#####################################################################################################
path_dir <- '/media/patrick/Elements/Hyperthermia/Figures_Human_GBM/Ctrl_Comparison/'
...
f <- paste(fig_save_path, 'gsea__heatresponse.svg', sep='')
...
f <- paste(fig_save_path, 'gsea_apoptosis.svg', sep='')
pathway <- 'BP.GO_REGULATION_OF_INTRINSIC_APOPTOTIC_SIGNALING_PATHWAY'
svg(f)
plotEnrichment(active_list[[pathway]], geneList) + labs(title="APOPTOSOS")
dev.off()


f <- paste(fig_save_path, 'gsea_AGING.svg', sep='')
pathway <- 'BP.GO_AGING'
svg(f)
plotEnrichment(active_list[[pathway]], geneList) + labs(title="AGING")
dev.off()
dev.off()


f <- paste(fig_save_path, 'gsea_ROS.svg', sep='')
pathway <- 'BP.GO_RESPONSE_TO_REACTIVE_OXYGEN_SPECIES'
svg(f)
plotEnrichment(active_list[[pathway]], geneList) + labs(title="ROS")
dev.off()
dev.off()
