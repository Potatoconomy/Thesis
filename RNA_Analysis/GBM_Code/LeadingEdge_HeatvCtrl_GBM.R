...
fig_save_path <- '/home/patrick/Desktop/Masters Thesis Figures/Unedited/LeadingEdge/'
...
preparation_function1 <- function(DESeqName){
  ...
  fgseaRes <- fgsea(pathways = pathways_list, 
                    ...
                    minSize  = 15,
                    maxSize  = 250,
                    nperm=1000000)  # altering minSize basically p hacks. :/
...
interesting_pathways <- function(FGSEA) {
  ...
  negative_pathways <- c('BP.GO_REGULATION_OF_INFLAMMATORY_RESPONSE','BP.GO_REGULATION_OF_LEUKOCYTE_PROLIFERATION',
                         'BP.GO_MYELOID_LEUKOCYTE_MIGRATION','BP.GO_LYMPHOCYTE_ACTIVATION_INVOLVED_IN_IMMUNE_RESPONSE',
                         'BP.GO_GRANULOCYTE_MIGRATION','BP.GO_DNA_REPLICATION')
  
  combined_pathways <- c(apoptosis_pathways, growth_pathways, heat_pathways, negative_pathways)
  
  #fgsea_metabolism <- FGSEA[FGSEA$pathway %in% metabolism_pathways]
  fgsea_apoptosis <- FGSEA[FGSEA$pathway %in% apoptosis_pathways]
  fgsea_growth <- FGSEA[FGSEA$pathway %in% growth_pathways]
  fgsea_heat <- FGSEA[FGSEA$pathway %in% heat_pathways]
  fgsea_combined <- FGSEA[FGSEA$pathway %in% combined_pathways]
  
  return(list(fgsea_apoptosis, fgsea_growth, fgsea_heat, fgsea_combined))
}
...
make_many <- function(fgsea_apoptosis, fgsea_growth, fgsea_heat, fgsea_combined){
  # Calls function: make_heatmap for all pathways
  anno_min = -2
  anno_max = 2
  c(X, nes, p_vals.pathways, p_vals.genes) %<-% make_heatmap(DF = fgsea_apoptosis, 
                                                             LFC = DESeqObject, 
                                                             fname = 'apoptosis',
                                                             anno_min = anno_min,
                                                             anno_max = anno_max)
  
  c(X, nes, p_vals.pathways, p_vals.genes) %<-% make_heatmap(DF = fgsea_heat, 
                                                             LFC = DESeqObject, 
                                                             fname = 'heat',
                                                             anno_min = anno_min,
                                                             anno_max = anno_max)
  
  c(X, nes, p_vals.pathways, p_vals.genes) %<-% make_heatmap(DF = fgsea_growth, 
                                                             LFC = DESeqObject, 
                                                             fname = 'growth',
                                                             anno_min = anno_min,
                                                             anno_max = anno_max)
  
  # c(X, nes, p_vals.pathways, p_vals.genes) %<-% make_heatmap(DF = fgsea_negative,
  #                                                            LFC = DESeqObject,
  #                                                            fname = 'negative',
  #                                                            anno_min = anno_min,
  #                                                            anno_max = anno_max)
...
#c(DESeqObject, geneList, pathways_list, fgseaRes, anno_max, anno_min) %<-% preparation_function1('lfc_healthy')

c(DESeqObject, geneList, pathways_list, fgseaRes, anno_max, anno_min) %<-% preparation_function1('lfc_tumor')

################################################
##~ Create Heatmaps ~###########################
################################################
# MAKE SURE TO NOT OVERWRITE OLD FILES. AFTER MAKING FIGURES, MOVE THEM TO DIFFERENT DIRECTORY
# AINT NOBODY GOT TIME TO MAKE PROPER CODE
c(fgsea_apoptosis, fgsea_growth, fgsea_heat, fgsea_combined) %<-% interesting_pathways(fgseaRes)

make_many(fgsea_apoptosis, fgsea_growth, fgsea_heat, fgsea_combined)
